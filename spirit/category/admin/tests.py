...
class AdminViewTest(TestCase):
    ...
    def test_category_create(self):
        ...
        utils.login(self)
        form_data = {
            "parent": "", "title": "foo", "description": "", "order": 10,
...
class AdminFormTest(TestCase):
    ...
    def test_category_invalid_parent(self):
        ...
        form = CategoryForm(data=form_data, instance=self.category, user=self.user)
        self.assertEqual(form.is_valid(), False)
        self.assertNotIn('parent', form.cleaned_data)
    ...
    def test_category_updates_reindex_at(self):
        ...
        self.assertEqual(
            category.reindex_at,
            yesterday)
        form = CategoryForm(instance=category, data=form_data, user=self.user)
        self.assertEqual(form.is_valid(), True)
        form.save()
        self.assertGreater(
